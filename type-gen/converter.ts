  ...
  const ammoExport = ts.createFunctionDeclaration(
    ...
    /* parameters     */ [
      ts.createParameter(
        ...
        ts.createTypeReferenceNode("T", [])
      ),
    ],
    /* type           */ ts.createTypeReferenceNode("Promise", [
      ts.createIntersectionTypeNode([
        ts.createTypeReferenceNode("T", []),
        ts.createTypeQueryNode(ts.createIdentifier("Ammo")),
      ]),
    ]),
    /* body           */ undefined
  );
    ...
    inheritance.push(
      ts.createHeritageClause(ts.SyntaxKind.ExtendsKeyword, [
        ts.createExpressionWithTypeArguments(
          undefined,
          ts.createIdentifier(idl.inheritance)
        ),
      ])
    );
  }
